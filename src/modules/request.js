...
export default {
    ...
    collection(username) {
        return this.xml(`${endpoint}collection?username=${username}&own=1&subtype=boardgame&excludesubtype=boardgameexpansion`)
            .then(root => {
                let collection = {};
    ...
    plays(username) {
        return this.xml(`${endpoint}plays?username=${username}&own=1`)
            .then(root => {
                let plays = {};
    ...
    fetch(username) {
        return Promise.all([this.collection(username), this.plays(username)]).then((values) => {
